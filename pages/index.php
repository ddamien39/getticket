...
<body>
    <section class="slidebar">
                    ...
                    <li>
                        <a href="/home.php" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                            <svg class="shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white"
                                ...
                                <path d="M10 2.5l7.5 7.5H15v7.5H5V10H2.5z" />
                            </svg>
        ...
        $ticketsCount = $pdo->query("SELECT COUNT(*) FROM ticket_replies")->fetchColumn();
        $usersCount = $pdo->query("SELECT COUNT(*) FROM users")->fetchColumn();
        $messagesCount = $pdo->query("SELECT COUNT(*) FROM inbox_messages")->fetchColumn();



        // Récupérer les 5 derniers tickets
        $stmt = $pdo->query("SELECT id, title, status FROM tickets ORDER BY created_at DESC LIMIT 5");
